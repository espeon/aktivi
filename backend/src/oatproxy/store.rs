...
impl PgStore {
    pub fn builder(db: PgPool) -> PgStoreBuilder {
        PgStoreBuilder {
            db,
            signing_key: None,
        }
...
#[async_trait]
impl OAuthSessionStore for PgStore {
    ...
    async fn consume_pending_auth(&self, code: &str) -> OatResult<Option<PendingAuth>> {
        ...
        if let Some(row) = row {
            // Delete the auth
            sqlx::query("DELETE FROM oatproxy_pending_auths WHERE code = $1")
                .bind(code)
                .execute(&self.db)
                .await
                .map_err(|e| jacquard_oatproxy::error::Error::StorageError(e.to_string()))?;
    ...
    async fn consume_downstream_client_info(
        ...
        if let Some(row) = row {
            ...
            sqlx::query("DELETE FROM oatproxy_downstream_clients WHERE did = $1")
                .bind(did)
                .execute(&self.db)
                .await
                .map_err(|e| jacquard_oatproxy::error::Error::StorageError(e.to_string()))?;
    ...
    async fn consume_par_data(&self, request_uri: &str) -> OatResult<Option<PARData>> {
        ...
        if let Some(row) = row {
            // Delete the PAR data
            sqlx::query("DELETE FROM oatproxy_par_data WHERE request_uri = $1")
                .bind(request_uri)
                .execute(&self.db)
                .await
                .map_err(|e| jacquard_oatproxy::error::Error::StorageError(e.to_string()))?;
...
#[async_trait]
impl jacquard_oauth::authstore::ClientAuthStore for PgStore {
    fn get_session(
        &self,
