...
function EventsPage() {
  const client = useXrpc();

  const { data, isLoading, error } = useQuery({
    queryKey: ["events"],
    queryFn: async () => {
      const response = await client.get("co.aktivi.event.getEvents", {
        params: {
          limit: 50,
        ...
        {eventsByDate.length === 0 && (
          <div className="text-center py-24">
            ...
            <p className="text-muted-foreground max-w-md mx-auto">
              events from around the Atmosphere will appear here, once there are
              any.
            </p>
          </div>
        )}
